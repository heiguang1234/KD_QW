...
def load_data():
    benign_permission_2023_csv_path = "/home/cl/cl_mac_workspace/process_2023_2024/features/benign_permission_2023.csv"
    malicious_permission_2023_csv_path = "/home/cl/cl_mac_workspace/process_2023_2024/features/malicious_permission_2023.csv"

    benign_permission_2024_csv_path = "/home/cl/cl_mac_workspace/process_2023_2024/features/benign_permission_2024.csv"
    malicious_permission_2024_csv_path = "//home/cl/cl_mac_workspace/process_2023_2024/features/malicious_permission_2024.csv"

    df = pd.read_csv(benign_permission_2023_csv_path,  header=None)
    print(df.values.shape)

    benign_permission_2023_x = df[df.columns[1:]].values
    benign_permission_2023_y = [0] * benign_permission_2023_x.shape[0]

    df = pd.read_csv(malicious_permission_2023_csv_path,  header=None)
    malicious_permission_2023_x = df[df.columns[1:]].values
    malicious_permission_2023_y = [1] * malicious_permission_2023_x.shape[0]

    df = pd.read_csv(benign_permission_2024_csv_path,  header=None)
    benign_permission_2024_x = df[df.columns[1:]].values
    benign_permission_2024_y = [0] * benign_permission_2024_x.shape[0]        

    df = pd.read_csv(malicious_permission_2024_csv_path,  header=None)
    malicious_permission_2024_x = df[df.columns[1:]].values
    malicious_permission_2024_y = [1] * malicious_permission_2024_x.shape[0]

    
    X_2023 = np.vstack((benign_permission_2023_x, malicious_permission_2023_x)).astype(np.float32)
    y_2023 = np.hstack((benign_permission_2023_y, malicious_permission_2023_y)).astype(np.float32)  

    X_2024 = np.vstack((benign_permission_2024_x, malicious_permission_2024_x)).astype(np.float32)
    y_2024 = np.hstack((benign_permission_2024_y, malicious_permission_2024_y)).astype(np.float32)
...
def teacher_predict(dataloader):
    ...
    # 1. 加载模型
    tea = Teacher(1756, output_dim_list=[1024, 512, 256, 128, 64, 32])
    pkl_path = "/home/cl/cl_mac_workspace/KD_QW3/Experiment_main/KD_Model/pre_per_1757/pre_teacher_0.pth"
...
def student_predict(dataloader):
    # ========= 1. 固定配置（工程内常量） =========
    CKPT_PATH = "/home/cl/cl_mac_workspace/KD_QW3/Experiment_main/KD_Model/pre_per_1757/pre_student_0.pth"
    INPUT_DIM = 1756
    N_CLASSES = 2
