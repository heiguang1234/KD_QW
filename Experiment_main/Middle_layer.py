...
from Fine_tune import kfold_training as fine_kd
...
def kfold_training(file_name, file_2022, kfold_seed, student_param_list,
    ...
    for k, (train_index, test_index) in enumerate(kfold.split(X)):
        ...
        test_loader_2022 = torch.utils.data.DataLoader(test_dataset_2022,
                                                       ...
                                                       shuffle=True)
        # 一次正常知识蒸馏，做一次test
        student_net, teacher_net = train_one_middle(k, train_loader,
                                                    student_param_list,
    ...
    middle_acc = get_average_reault(all_student_indicators,
...
def distillation_loss(student_output, teacher_output, label):
    student_middle_output_list,  student_hard_y = student_output
    teacher_middle_output_list,  teacher_hard_y = teacher_output
    # 获得teacher_idx
    idx_list = []
...
def train_one_middle(k, train_loader, student_param_list, teacher_param_list,
    ...
    student_net_middle.load_state_dict(
        torch.load('Experiment_main/KD_Model/{}/pre_student_'.format(file_name) + str(k) + '.pth'))
    teacher_net.load_state_dict(
        torch.load('Experiment_main/KD_Model/{}/pre_teacher_'.format(file_name) + str(k) + '.pth'))
    ...
    torch.save(student_net_middle.state_dict(),
            'Experiment_main/Middle_Model/pre_q/{}/pre_student_'.format(file_name) + str(k) + '.pth')
    student_net_middle.eval()
    student_net_middle = torch.ao.quantization.convert(student_net_middle,
                                                       inplace=True)
    torch.save(student_net_middle.state_dict(),
               'Experiment_main/Middle_Model/{}/pre_student_'.format(file_name) + str(k) + '.pth')
    torch.save(teacher_net.state_dict(),
               'Experiment_main/Middle_Model/{}/pre_teacher_'.format(file_name) + str(k) + '.pth')
...
if __name__ == '__main__':
    ...
    # for file_name in ['pre_per_1757']:
        kfold_training(
            ...
            batch_size=256,  
            torch_seed=0,     
            path='Experiment_main/Middle_result'
        )
