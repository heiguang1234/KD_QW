...
def kfold_training(file_name, file_2022, kfold_seed, student_param_list,
    ...
    for k, (train_index, test_index) in enumerate(kfold.split(X)):
        ...
        test_loader_2022 = torch.utils.data.DataLoader(test_dataset_2022,
                                                       ...
                                                       shuffle=True)
        # 一次正常知识蒸馏，做一次test
        student_net, teacher_net = train_one_fine(k, train_loader,
                                                  student_param_list,
    ...
    fine_acc = get_average_reault(all_student_indicators,
...
def train_one_fine(k, train_loader, student_param_list, teacher_param_list,
    ...
    optimizer = torch.optim.Adam(itertools.chain(student_net_fine.parameters(),teacher_net.parameters()),
                                 lr=lr)
    ...
    student_net_fine = torch.ao.quantization.convert(student_net_fine,
                                                     inplace=True)
    torch.save(student_net_fine.state_dict(),
               'Experiment_main/Fine_Model/{}/pre_student_'.format(file_name) + str(k) + '.pth')
    torch.save(teacher_net.state_dict(),
               'Experiment_main/Fine_Model/{}/pre_teacher_'.format(file_name) + str(k) + '.pth')
